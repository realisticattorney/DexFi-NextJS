...
const MenuPanel = ({ currencies, section }) => {
  ...
  const handleMenuItemClick = async (event, index, menuItem) => {
    if (menuItem === 1) {
      if (index === outputToken[1]) {
        await handleMenuItemSwitch(inputToken[1], outputToken[1]);
      } else {
  ...
  const handleMenuItemSwitch = async (prevSelected, newSelected) => {
    ...
    const prevInputOne = inputOne;
    const prevInputTwo = inputTwo;
    await callExchange(inputTwo, 2)
    await callExchange(prevInputOne, 2)
    // setInputTwo(prevInputOne);
  };
