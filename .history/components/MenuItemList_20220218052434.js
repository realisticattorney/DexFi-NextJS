...
const MenuItemList = ({ handleOpen, handleClose, currencies, token, open, inputTwo, inputOne }) => {
  return (
    <div className="flex flex-col space-y-2 p-5">
      <button onClick={handleOpen} className="flex items-center">
        <Image
          src={token.currentToken[0].logoURI}
          height={24}
          width={24}
          quality={50}
          alt=""
          ...
          <Paper sx={modalstyle}>
            <MenuList>
              ...
              {currencies.map((currency, index) => (
                <MenuItem
                  key={currency.symbol}
                  disabled={index === token.currentToken[1]}
                  selected={index === token.currentToken[1]}
