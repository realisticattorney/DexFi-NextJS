...
const MenuPanel = ({ currencies }) => {
  ...
  const handleInputOneChange = (event) => {
    ...
    if (event.target.value > 0) {
      callExchange(event.target.value, event.target.id); // eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee;
  ...
  return (
    <>
      ...
      />
      {inputOne > 0 && (
        <PriceEstimator inputOne={inputOne} inputTwo={inputTwo} inputToken={inputToken} outputToken />
      )}
      <div className="h-10">
