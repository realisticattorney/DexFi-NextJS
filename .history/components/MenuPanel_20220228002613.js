...
const MenuPanel = ({ currencies, section }) => {
  ...
  async function callExchange(input, id) {
    ...
    if (callFunction === 'TokenToTokenSwap') {
      ...
      amount =
        id === '1'
          ? ethers.utils.formatEther(await exchangeCurrent.contract.getEthAmount(price))
          : ethers.utils.formatEther(await exchangeCurrent.contract.getTokenAmount(price));
    } else {
      amount =
        id === '1'
          ? ethers.utils.formatEther(await exchangeCurrent.contract.getTokenAmount(price))
