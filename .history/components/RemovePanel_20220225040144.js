...
const RemovePanel = ({ address, currency }) => {
  ...
  useEffect(() => {
    const loadExchange = async () => {
      ...
      returnsEstimator(userLPTokens, exchange, exchangeBalance, totalSupply, getReserve);
  ...
  const returnsEstimator = useCallback(
    async (lps, exchange, exchangeBalance, totalSupply, getReserve) => {
       const ethWithdrawn = (exchangeBalance * lps) / totalSupply;
       const tokenWithdrawn = (getReserve )
    },
    [exchange, address]
  );
