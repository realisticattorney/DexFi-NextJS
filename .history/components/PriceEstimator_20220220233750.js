...
const PriceEstimator = ({
  ...
  useEffect(() => {
    ...
    loadLiquidity();
  }, [callBondingCurve, provider, inputOne, exchange]);
            ...
            {inputOne > 0 && (
              <div className="flex">
                <div className="text-center">
                  ...
                  <h1>{`${inputToken[0].symbol} per ${outputToken[0].symbol}`}</h1>
                </div>
                <div className="text-center">
                  <h1 className="truncate text-sm">{`${
                    (inputOne / inputTwo).toString().length > 9
                ...
                <div className='text-center'><h1 className="truncate text-sm">{`${
                  (inputOne / inputTwo).toString().length > 9
                    ? (inputOne / inputTwo).toString().substring(0, 10)
                    : (inputOne / inputTwo).toString()
                }`}</h1>
                <h1 className='text-sm font-medium'>Share of Pool</h1>
                </div>
              </div>
            )}
          </div>
