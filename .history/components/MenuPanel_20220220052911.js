...
const MenuPanel = ({ currencies, section }) => {
  ...
  async function callBondingCurve(input, id) {
    ...
    //else, getReserve() / provider.balanceof(exchange) set id === 2 ? input there, input *  getReserve() / provider.balanceof(exchange and the other input getReserve() / provider.balanceof(exchange) / input
