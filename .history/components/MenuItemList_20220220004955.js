...
const MenuItemList = ({
  ...
  const handleInputChange = (event) => {
    ...
    if (event.target.value > 0) {
      ...
      handleInputToken(null);
      handleOutputToken(null);
    } else if (event.target.value === '00') {
      handleInputToken(inputOne);
      handleOutputToken(inputTwo);
    } else {
      handleInputToken(event.target.value);
      handleOutputToken(event.target.value);
    }
  };
