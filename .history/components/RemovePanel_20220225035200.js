...
const RemovePanel = ({ address, currency }) => {
  ...
  useEffect(() => {
    const loadExchange = async () => {
      ...
      returnsEstimator(userLPTokens, exchange.address, );

      const getReserve = ethers.utils.formatEther(await exchange.getReserve());

      ethWithDrawn = setExchange(exchange);
...
const returnsEstimator = useCallback(async (lps, exchangeAddress) => {
   const ethWithDrawn = ethers.utils.formatEther(
