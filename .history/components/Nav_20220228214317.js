...
const Nav = () => {
  const { connect, isWeb3Loaded, isUserWalletConnected, exchangeBunny, provider } =
    useWeb3();
  ...
  const ethAccountBalance = useCallback(() => {
    if (account && exchangeBunny) {
      return exchangeBunny.methods.balanceOf(account).call();
    }
  }, [account, exchangeBunny]);
