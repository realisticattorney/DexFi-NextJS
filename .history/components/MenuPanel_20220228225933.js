...
const MenuPanel = ({ currencies, section }) => {
  ...
  const handleMenuItemSwitch = useCallback(
    ...
    [inputOne, inputTwo, currencies]
  );

  const handleMenuItemCall = useCallback(
    (event, index, menuItem) => {
      if (menuItem === 1) {
        handleInputChange(event, index, menuItem);
      } else {
        handleOutputChange(event, index, menuItem);
      }
    },
    [handleInputChange]
  );

  useEffect(() => {}, [handleMenuItemSwitch]);
  ...
  const handleInputChange = useCallback(
    ...
    [inputOne, inputTwo, callExchange, callBondingCurve, section]
  );
