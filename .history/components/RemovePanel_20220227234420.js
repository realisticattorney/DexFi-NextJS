...
const RemovePanel = ({ address, currency, backCurrency }) => {
  ...
  const returnsEstimator = useCallback(
    (userLpsToRemove) => {
      ...
      const ethWithdrawn = (exchangeCurrent.balance * lps) / exchangeCurrent.contract;
      const tokenWithdrawn = (tokenReserve * lps) / exchangeCurrent.contract;

      setExpectedWithdrawn([ethWithdrawn, tokenWithdrawn]);
    },
    [userLps, exchangeCurrent.balance, exchangeCurrent.contract, tokenReserve]
  );
    ...
    <div className="flex flex-col p-5">
      ...
      <div className="mt-2 bg-gray-100 w-full p-4 border border-slate-200 rounded-2xl">
        <div className="flex justify-between">
          ...
          <p className="font-medium text-sm text-dexfi-grayviolet">
            {exchangeCurrent.balance / tokenReserve} {backCurrency.symbol}
          </p>
        </div>
        <div className="flex justify-between mt-1">
          <h1 className="font-medium text-sm text-dexfi-grayviolet">1 WETH</h1>
          <p className="font-medium text-sm text-dexfi-grayviolet">
            {tokenReserve / exchangeCurrent.balance} {currency.symbol}
          </p>
        </div>
      </div>
      <button
