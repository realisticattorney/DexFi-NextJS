...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useCallback(async () => {
    ...
    const userLiquidityEvents = events.result.filter(
      ({ data }) => data.provider === user.get('ethAddress')
    );
    const sumWithInitial = events1.result.reduce(
      (previousValue, currentValue) => ({ ethAmount: previousValue.data.ethAmount + currentValue.data.ethAmount, tokenAmount: previousValue.data.tokenAmount + currentValue.data.tokenAmount }),
      initialValue
    );
    
    const userLps = userLiquidityEvents.map(({ data }) => ({
