...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useCallback(async () => {
    ...
    const userLiquidityEvents = events1.result.filter(
      ({ data }) => data.provider === user.get('ethAddress')
    ).concat(events2.result.filter(({ data }) => data.provider === user.get('ethAddress')))
      .map(({ data }) => { return { ...data, symbol: data.symbol.toUpperCase() } });
    ...
    const sumWithInitial = userLiquidityEvents.reduce(
      (previousValue, { data: { ethAmount, tokenAmount } }) => ({
        ethAmount: previousValue.ethAmount + parseInt(ethAmount),
        tokenAmount: previousValue.tokenAmount + parseInt(tokenAmount),
      }),
