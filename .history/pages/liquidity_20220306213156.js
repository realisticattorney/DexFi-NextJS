...
import { useState, useEffect, useCallback, useMemo } from 'react'; //hooks
...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useMemo(async () => {
    const options1 = {
      chain: 'rinkeby',
    ...
    let events = await Web3Api.native.getContractEvents(options1);
    events = events.concat(await Web3Api.native.getContractEvents(options2));
