...
export default function Home(props) {
  ...
  const [inputTo, setinputTo] = useState([currencies[0], 0]);
  ...
  const handleMenuItemClick = async (event, index, menuItem) => {
    ...
    if (menuItem === 1) {
      if (index === toSwapCurrency[1]) {
        handleMenuItemSwitch(inputTo[1], toSwapCurrency[1]);
        isSwitch = true;
      } else {
        setinputTo([currencies[index], index]);
      }
      handleClose();
    } else {
      if (index === inputTo[1]) {
  ...
  const handleMenuItemSwitch = (prevSelected, newSelected) => {
    ...
    setinputTo([currencies[newIndex], newIndex]);
                ...
                <Paper sx={modalstyle}>
                  <MenuList>
                    ...
                    {currencies.map((currency, index) => (
                      <MenuItem
                        key={currency.symbol}
                        disabled={index === inputTo[1]}
                        selected={index === inputTo[1]}
                        onClick={(event) =>
