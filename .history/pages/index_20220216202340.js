  ...
  useEffect(() => {
    async function loadExchange(a, b, c, d) {
      ...
      if (exchange === null) {
        ...
        let menuOneShouldntBeTheExchange = a[1] === 1 ? true : false;
        let hasntMenuOneChanged =
          exchangeTokenAddress === a[0].address ? true : false;
        if (
          (!menuOneShouldntBeTheExchange && hasntMenuOneChanged) ||
          (!menuOneShouldntBeTheExchange && exchangeTokenAddress === b[0].address)
        ) {
        ...
        if (!menuOneShouldntBeTheExchange) {
          console.log('menuOneShouldntBeTheExchange');
