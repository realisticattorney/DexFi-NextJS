  ...
  useEffect(() => {
    if (isUserWalletConnected) {
      const promises = currencies.map(async (currency) => {
        const mappedExchange = await registry.getExchange(currency.address);
        
        const lp = await mappedExchange.balanceOf(currency.address);
        const totalSupply = await lp.getTotalSupply();
        return {
          ...currency,
          totalSupply,
        };
