...
export default function Home(props) {
  ...
  const [exchangeSwitch, setExchangeSwitch] = useState("");
  ...
  const exchangeHandler = useCallback(() => {
    if (inputToken.currentToken[1] === outputToken.prevToken[1]) {
      return "switch"
    } else if()
  ...
  const handleMenuItemClick = async (event, index, menuItem) => {
    ...
    if (menuItem === 1) {
      if (index === outputToken.currentToken[1]) {
        ...
        setExchangeSwitch(inputToken.currentToken[1].address)
      }
      handleClose();
    } else {
