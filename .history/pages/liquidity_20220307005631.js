    ...
    async (mappedExchangeAddress) => {
      ...
      let events = LiquidityEvents.reduce(
        (
          ...
          ethAmount: symbol
            ? previousValue.ethAmount - parseInt(ethers.utils.formatEther(ethAmount))
            : previousValue.ethAmount + parseInt(ethers.utils.formatEther(ethAmount)),
          tokenAmount: symbol
            ? previousValue.tokenAmount - parseInt(ethers.utils.formatEther(tokenAmount))
            : previousValue.ethAmount + parseInt(ethers.utils.formatEther(tokenAmount)),
          userEthAmount:
            provider === user.get('ethAddress')
              ? symbol
                ? previousValue.userEthAmount - parseInt(ethers.utils.formatEther(ethAmount))
                : previousValue.userEthAmount + parseInt(ethers.utils.formatEther(ethAmount))
              : previousValue.userEthAmount,
          userTokenAmount:
            provider === user.get('ethAddress')
              ? symbol
                ? previousValue.userTokenAmount - parseInt(ethers.utils.formatEther(tokenAmount))
                : previousValue.userTokenAmount + parseInt(ethers.utils.formatEther(tokenAmount))
      ...
      events = {
        ethAmount: ethers.utils.formatEther(events.ethAmount),
        tokenAmount: ethers.utils.formatEther(events.tokenAmount),
        userEthAmount: ethers.utils.formatEther(events.userEthAmount),
        userTokenAmount: ethers.utils.formatEther(events.userTokenAmount),
      };
      return events;
    },
    [user, Web3Api.native]
