  ...
  const fetchContractEvents = useCallback(
    async (currency) => {
      ...
      const events = LiquidityEvents.reduce(
        (previousValue, { data: { ethAmount, tokenAmount, symbol,provider } }) => ({
          ethAmount: symbol
          ...
          tokenAmount: symbol
            ...
            userEthAmount: provider === user.get('ethAddress') ? previousValue.userEthAmount + parseInt(ethAmount) : previousValue.userEthAmount,
        }),
    ...
    [user, Web3Api.native, registry]
  );

  useEffect(() => {
    if (userLps.length === 0 && user && registry) {
      const promises = currencies.map(async (currency) => {
        const userLPTok = await fetchContractEvents(currency);
