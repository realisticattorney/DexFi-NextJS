...
export default function Home(props) {
  ...
  const handleMenuItemClick = async (event, index, menuItem) => {
    ...
    let exchangeIs = exchangeTokenAddress === exchangeCurrency[0].address ? "top" : "bottom";
    if(exchangeIs === "top") {
      let isBottomEth = toSwapCurrency[1] === 1 ? true : false;
      let movingToEth = index === 1 ? true : false;
      if(menuItem === 1) {
        if(isBottomEth) {
          //cambio de exchange, mantengo tokenToEth
        } else{
          if(movingToEth) {
            //cambio de exchange al de abajo, paso de tokenToToken a EthToToken
          } else {
      ...
      //   if(movingToEth) {
      //     //swapear el de abajo por el de arriba
      //     if(isBottomEth) {
      //       //mantengo exchange, cambio tokenToEth a ethToToken
      //     } else {
      //       //cambio exchange, mantengo tokenToEth
      //     }
      //   } else {
      //     if(isBottomEth) {
      //       //cambio exchange, mantengo tokenToEth 
      //     } else {
      //       //cambio exchange, mantengo tokenToToken
      //     }
      // }
    } else {//si es el segundo el que se mueve
        if(movingToEth) {
          //mantengo exchange, paso de tokenToToken a tokenToEth
        } else {
  ...
  //     if (exchangeTokenAddress === exchangeCurrency[0].address) {//no puede ser ethereum
  //       if (index === 1) {//si va a pasar a ser ethereum
  //         if (toSwapCurrency[1] === 1) { //si el de abajo era ethereum
  //           //mantengo exchange, paso de tokenToEth a ethToToken
  //         } else {//si el de abajo no era ethereum
  //           //cambio de exchange, paso de tokenToToken a ethToToken
  //         }
  //       } else { //si no va a pasar a ser ethereum
  //         if(index === toSwapCurrency[1]) { //y va a pasar a ser el token de abajo
  //           //cambio de exchange, mantengo tokenToToken
  //         } else {//si va a pasar a ser otro token
  //           //mantengo exchange, 
  //         }
  //       }
  //     } else {//tiene que ser ethereum
  //       //cambio de exchange al index, paso de ethToToken a tokenToToken
  //     }
  //     handleClose();
  //   } else {
