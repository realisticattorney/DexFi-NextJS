  ...
  async function callExchange(input, id) {
    ...
    if (selectedIndex !== 1) {
      if (selectedIndexSecond === 1) {
        amount = id === "outlined-number-1" ? ethers.utils.formatEther(await exchange.getEthAmount(price2)) : ethers.utils.formatEther(await exchange.getTokenAmount(price2));
      } else {
        // amount = ethers.utils.formatEther(await exchange.getEthAmount(price2));
      }
    } else {
      amount = id === "outlined-number-1" ? ethers.utils.formatEther(await exchange.getTokenAmountgetEthAmount(price2)) : ethers.utils.formatEther(await exchange.getTokenAmount(price2));
    }
