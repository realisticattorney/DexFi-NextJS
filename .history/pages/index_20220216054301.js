    ...
    async function loadExchange(a, b, c, d) {
      ...
      if (exchange === null) {
        ...
        if (isMenuOneTheExchange) {
          isMenuTwoEth = b[1] === 1? true : false;
          hasMenuOneChanged = exchangeTokenAddress === a[0].address ? true : false;
          if (isMenuTwoEth) {
            if(hasMenuOneChanged) {
            setExchange(
                    new ethers.Contract(await d.getExchange(a[0].address, Exchange.abi, provider)
                  ));
                } else {
              setSwapType('TokenToEthSwap');
      }
          ...
          if(hasMenuOneChanged) {
            ...
            setSwapType('TokenToTokenhSwap');
          } else {
          }
        }
      } else {
        // hasMenuTwoChanged = exchangeTokenAddress === b[0].address ? true : false;
        // if(hasMenuTwoChanged) {
          setExchange(
            new ethers.Contract(await d.getExchange(a[0].address, Exchange.abi, provider)
          ));
      } else {
