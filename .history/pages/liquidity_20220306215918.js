...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useCallback(async () => {
    ...
    const userLiquidityEvents = await events1.result.filter(
    ...
    const sumWithInitial = userLiquidityEvents.data.reduce(
      (previousValue, currentValue) => ({
        ethAmount: previousValue.ethAmount + currentValue.ethAmount,
        tokenAmount:
          previousValue.tokenAmount + currentValue.tokenAmount,
      }),
      0
    );
