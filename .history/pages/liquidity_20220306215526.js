...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useCallback(async () => {
    ...
    const sumWithInitial = events1.result.reduce(
      (previousValue, currentValue) => ({ ethAmount: previousValue.data.ethAmount + currentValue.data.ethAmount, usdAmount: previousValue.data.usdAmount + currentValue.data.usdAmount }),
      initialValue
    );
