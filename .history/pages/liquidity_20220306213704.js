...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useCallback(async () => {
    ...
    let events = await Web3Api.native.getContractEvents(options1).then(
      //concatenate the two events
      (events1) =>
        Web3Api.native.getContractEvents(options2).then((events2) => {
    );
    // let events2 = events.concat(
    //   await Web3Api.native.getContractEvents(options2)
    // );
