        ...
        <div className="flex-col relative w-[436px] min-h-[273px] bg-white rounded-3xl border shadow-sm">
          ...
          {isUserWalletConnected && userLps.length > 0 ? (
            userLps.map((currency, index) => (
              <div key={index} className={`flex px-5 mb-4 justify-between ${index % 1 ? "bg-slate-500 : ""}`}>
              <div className='flex'>
                <Image
                  ...
                  alt=""
                />
                <h1 className="ml-2 font-semibold text-dexfi-violet">{currency.symbol}</h1>
                </div>
                <p className="ml-3 font-bold text-violet-700">${currency.userLPTokens}</p>
              </div>
            ))
          ) : (
