...
export default function Liquidity(props) {
  ...
  const fetchContractEvents = useCallback(async () => {
    ...
    let events1 = await Web3Api.native.getContractEvents(options1);
    //concat events from second contract
    let events2 = await Web3Api.native.getContractEvents(options2);
    //use reducer to concat events

    const userLiquidityEvents = events1.result
      .filter(({ data }) => data.provider === user.get('ethAddress'))
      .concat(
        events2.result.filter(
          ({ data }) => data.provider === user.get('ethAddress')
        )
      )
      .map(({ data }) => {
        return { ...data, symbol: 'remove' };
      });
    console.log('userLiquidityEvents', userLiquidityEvents);
    const sumWithInitial = userLiquidityEvents.reduce(
      (previousValue, { data: { ethAmount, tokenAmount } }) => ({
        ethAmount: data.symbol ? previousValue.ethAmount - parseInt(ethAmount) : previousValue.ethAmount + parseInt(ethAmount),
        tokenAmount: data.symbol ? previousValue.tokenAmount - parseInt(tokenAmount) : previousValue.ethAmount - parseInt(ethAmount),
      }),
