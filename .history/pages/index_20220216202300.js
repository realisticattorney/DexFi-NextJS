  ...
  useEffect(() => {
    async function loadExchange(a, b, c, d) {
      ...
      if (exchange === null) {
        ...
        let menuShouldntBeOneTheExchange = a[1] === 1 ? false : true;
        let hasntMenuOneChanged =
          exchangeTokenAddress === a[0].address ? true : false;
        if (
          (isMenuOneTheExchange && hasntMenuOneChanged) ||
          (!isMenuOneTheExchange && exchangeTokenAddress === b[0].address)
        ) {
        ...
        if (isMenuOneTheExchange) {
          console.log('isMenuOneTheExchange');
