...
async function main() {
  ...
  let ETCExchange = await registry.createExchange(tokenTwo.address);
  let txReceiptTwo = await ETCExchange.wait();
  const [, ETCExchangeAddress] = txReceiptTwo.events[0].args;
  console.log('Receipt ETCExchangeAddress:', ETCExchangeAddress);
  const getETCExchangeAddress = await registry.getExchange(tokenTwo.address);
