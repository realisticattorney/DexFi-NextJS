...
async function main() {
  ...
  const getscammExchangeAddressExchangeAddress = await registry.getExchange(token.address);


  let USDCExchange = await registry.createExchange(token.address);
  let txReceiptTwo = await USDCExchange.wait();
  const [, exchangeAddress] = txReceiptTwo.events[0].args;
  console.log('Receipt ScammCoinExchangeAddress:', exchangeAddress);
